...
def parse_opts():
    ...
    parser.add_argument('--postfix', type=str, default='', help='Postfix of save folder. Use to add some running details')
    ...
    parser.add_argument('--dict_file', type=str, default='data/anet/vocabulary_activitynet_thres2.json', help='')
    ...
    parser.add_argument('--caption_loss_type', type=str, default='ce', choices=['ce', 'rl'])
    ...
    parser.add_argument('--word_context_modeling_type',  type=str, default='coot_afa')
    ...
    parser.add_argument('--enable_proposal_level_attention_loss', type=bool, default=False)
    parser.add_argument('--pl_att_loss_coef', type=float, default=0.5)
    ...
    # frame-word CL
    parser.add_argument('--enable_frame_word_cl', action='store_true', help='whether to use frame-word contrastive loss')
    parser.add_argument('--fw_cl_temperature', type=float, default=0.1, help = 'temperature ratio in infoNCE loss')
    parser.add_argument('--fw_cl_loss_coef', type=float, default=0.0, help = 'loss weight')
    ...
    # text reconstruction
    parser.add_argument('--enable_text_reconstruction', action='store_true',
                        help='use text reconstruction decoder')
    ...
    # Multi anno dataset train and eval settings
    parser.add_argument('--support_sample_event_training', type=bool, default=False)
    parser.add_argument('--min_sample_event_num', type=int, default=5)
    parser.add_argument('--max_sample_event_num', type=int, default=10)
    parser.add_argument('--enable_repeat_events', action='store_true', default=False)
    parser.add_argument('--need_sample_for_multi', type=bool, default=False)
    parser.add_argument('--eval_for_multi_anno', type=bool, default=False)
    ...
    parser.add_argument('--crop_loader_mode', type=str, default='fix')


    # reranking
    parser.add_argument('--ec_alpha', type=float, default=0.3)

    # prompt for temporal action localization
    ...
    parser.add_argument('--cached_tokens', type=str, default='anet/activitynet_train_ngrams_for_cider-idxs.p')
    ...
    # ***************************** Event ENCODER  *****************************
    # parser.add_argument('--esgn_event_encoder_type', type=str, choices=['basic', 'TSRM'], default='basic')
    # parser.add_argument('--esgn_hidden_dim', type=int, default=512, help='hidden size of all fc layers')
    # parser.add_argument('--esgn_position_encoding_size', type=int, default=100)

    #  ***************************** SELECTOR  *****************************
    parser.add_argument('--enable_selector', action='store_true')
    parser.add_argument('--detach_selector', action='store_true')
    parser.add_argument('--only_ft_selector_head', action='store_true')
    parser.add_argument('--selector_loss_coef', type=float, default=1.0)
    parser.add_argument('--enable_selector_rl', action='store_true')
    # parser.add_argument('--esgn_decoder_input_feats_type', type=str, default='C', choices=['C', 'E', 'C+E'],
    #                     help='C:clip-level features, E: event-level features, C+E: both')
    # parser.add_argument('--esgn_decoder_type', type=str, default="show_attend_tell",
    #                     choices=['show_attend_tell', 'hrnn', 'cmg_hrnn'])
    parser.add_argument('--esgn_rnn_size', type=int, default=512,
                        help='size of the rnn in number of hidden nodes in each layer')
    parser.add_argument('--esgn_num_layers', type=int, default=1, help='number of layers in the decoderRNN')
    parser.add_argument('--esgn_att_hid_size', type=int, default=512, help='the hidden size of the attention MLP')
    parser.add_argument('--esgn_drop_prob', type=float, default=0.5, help='strength of dropout in the Language Model RNN')
    parser.add_argument('--esgn_max_decoding_len', type=int, default=10, help='')
    parser.add_argument('--esgn_enable_rnn_bias', action='store_true')
    parser.add_argument('--esgn_input_dim', type=int, default=512*2, help='')

    parser.add_argument('--selector_scorer_types', type=str, nargs='+', default=['Meteor'], choices=['Meteor', 'CiderD'])
    parser.add_argument('--selector_scorer_weights', type=float, nargs='+', default=[1.])

    # ************** FOR GEBC task only************
    parser.add_argument('--enable_feature_diff', action='store_true')
    parser.add_argument('--support_object_feature', action='store_true')
    parser.add_argument('--object_feature_dim', type=int, default=2056)
    parser.add_argument('--object_feature_path_map',type=str, default='')
    parser.add_argument('--max_obj_number', type=int, default=10)
