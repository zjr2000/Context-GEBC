...
def main(opt):
    ...
    if opt.eval_mode == 'test':
        ...
        caption_scores, eval_loss = evaluate(model, criterion, contrastive_criterion, postprocessors, loader, out_json_path,
                         logger, alpha=opt.ec_alpha, dvc_eval_version=opt.eval_tool_version, device=opt.eval_device, debug=opt.eval_debug, skip_lang_eval=False, verbose=opt.show_all_results, eval_enable_zeroshot_tal=opt.eval_enable_zeroshot_tal)
