...
class EDVCdataset(Dataset):
    ...
    def __init__(self, anno_file, feature_folder, translator_json, is_training, proposal_type, opt):
        ...
        self.need_sample_for_multi = opt.need_sample_for_multi and is_training
        self.need_rebuild_feat_key = (opt.eval_for_multi_anno and not is_training) or opt.train_with_split_anno or (opt.enable_repeat_events and not is_training)
        self.need_sample_events = opt.support_sample_event_training and is_training
...
class PropSeqDataset(EDVCdataset):
    ...
    def __getitem__(self, idx):
        ...
        feat_key = key[3:] if self.need_rebuild_feat_key else key
        ...
        if self.need_sample_for_multi:
            gt_timestamps, captions = sample_for_multi_sentence(gt_timestamps, captions, 
                enable_repeat_events=self.opt.enable_repeat_events,sample_events=self.need_sample_events,
                min_sample_num=self.opt.min_sample_event_num, max_sample_num=self.opt.max_sample_event_num)
    ...
    def load_object_feats(self, key, timestamps, duration, max_obj_num=10, feat_dim=2054):
        if not self.support_object_feature:
            return None
        key = key[2:13]
