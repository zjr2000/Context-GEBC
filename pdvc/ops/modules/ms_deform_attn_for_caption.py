...
class MSDeformAttnCap(nn.Module):
    def __init__(self, d_model=256, n_levels=4, n_heads=8, n_points=4, opt=None):
        """
        ...
        if opt is not None and vars(opt).get('enable_pos_emb_for_captioner'):
            self.sampling_offsets = nn.Linear(3 * d_model, n_heads * n_levels * n_points)
            self.attention_weights = nn.Linear(3 * d_model, n_heads * n_levels * n_points)
        else:
