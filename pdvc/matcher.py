...
class HungarianMatcher(nn.Module):
    ...
    def __init__(self,
        ...
        self.cost_cl = cost_cl
        self.opt = opt
        self.enable_before_and_after = vars(self.opt).get('enable_before_and_after', False)

        # assert cost_class != 0 or cost_bbox != 0 or cost_giou != 0 or cost_cl!=0, "all costs cant be 0"

    def forward(self, outputs, targets, verbose=False, return_C=False):
        ...
        if self.enable_before_and_after:
            ...
            indices = []
            for i in range(bs):
                num_gts = targets[i]['labels'].shape[0]
                indicies_i = torch.arange(num_gts)
                indicies_j = torch.arange(num_gts)
                indices.append((indicies_i, indicies_j))
            rl_indices = indices
        ...
        with torch.no_grad():
            ...
            # Final cost matrix
            C = self.cost_bbox * cost_bbox + self.cost_class * cost_class + self.cost_giou * cost_giou + self.cost_cl * cost_cl
...
def build_matcher(args):
    return HungarianMatcher(cost_class=args.set_cost_class,
                            ...
                            cost_cl= vars(args).get('set_cost_cl', 0.),
                            opt=args,
                            )