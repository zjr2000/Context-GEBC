...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
                 ...
                 num_feature_levels=4, dec_n_points=4, enc_n_points=4, enable_before_and_after=False):
        super().__init__()

        self.enable_before_and_after = enable_before_and_after
        if self.enable_before_and_after:
    ...
    def prepare_decoder_input_proposal(self, gt_reference_points):
        device = gt_reference_points.device
        ...
        # tgt: (bsz, n, dim) n is the gt proposal num
        if self.enable_before_and_after:
            batch_size = tgt.shape[0]
            processed = []
            proposal_mask = gt_reference_points.sum(2) != 0
            for i in range(batch_size):
                real_num_queries = proposal_mask[i].sum().item()
                single_type_proposal_num = int(real_num_queries / 2)
                before_proposal = tgt[i,:single_type_proposal_num,:].unsqueeze(0)
                after_proposal = tgt[i,single_type_proposal_num:real_num_queries,:].unsqueeze(0)
                padding = tgt[i,real_num_queries:,:].unsqueeze(0)
                assert before_proposal.shape[1] == after_proposal.shape[1]
                before_proposal = before_proposal + self.type_embed(torch.zeros([1, before_proposal.size(1)], device=device).long())
                after_proposal = after_proposal + self.type_embed(torch.ones([1, after_proposal.size(1)], device=device).long())
                tgt_item = torch.cat([before_proposal, after_proposal, padding], dim=1)
                processed.append(tgt_item)
            tgt = torch.cat(processed, dim=0)
        return init_reference_out, tgt, reference_points, query_embed
...
class DeformableTransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, reference_points, src, src_temporal_shapes, src_level_start_index, src_valid_ratios,
                query_pos=None, src_padding_mask=None, query_padding_mask=None, disable_iterative_refine=False, support_object_feature=False, object_features=None, object_decoder=None):
        output = tgt
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if support_object_feature:
                output = output + object_decoder(output, object_features)
...
def build_deforamble_transformer(args):
    enable_before_and_after = vars(args).get('enable_before_and_after', False)
    return DeformableTransformer(
        ...
        enable_before_and_after=enable_before_and_after)
