...
class PDVC(nn.Module):
    ...
    def selector_parameters(self):
        params = []
        modules = [self.start_query, self.end_query, self.selector]
    ...
    def parallel_prediction_matched(self, dt, criterion, contrastive_criterion, hs, query_embed, init_reference,
        ...
        if self.aux_loss:
            ...
            for l_id in range(hs.shape[0]):
                ...
                indices = last_indices[0] if l_id == hs.shape[0] - 1 else aux_indices[l_id][0]
                hs_for_cap = torch.cat([hs_lid, query_embed], dim=-1) if vars(self.opt).get('enable_pos_emb_for_captioner', False) else hs_lid
                text_embed_for_cap = others['pre_proj_text_embed'][l_id] if self.opt.enable_text_reconstruction else None
        ...
        if self.enable_selector:
            hs_for_selector = torch.cat([hs[-1], query_embed], dim=-1) if vars(self.opt).get('enable_pos_emb_for_selector', False) else hs[-1]
            selector_loss = selecting(self.start_query, self.end_query, self.selector, hs_for_selector, last_indices[0], is_training=True)
            s_dict = {'loss_selector': selector_loss}
            loss.update(s_dict)
        return out, loss
    ...
    def caption_prediction(self, cap_head, dt, hs, reference, others, captioner_type, indices=None, text_embed=None):
        ...
        if self.opt.enable_text_reconstruction:
            cap_loss = cap_head.build_loss(cap_prob, caption_tensor.repeat(2, 1), caption_mask.repeat(2, 1))
            cap_cost = cap_loss
            vis_cap_loss = cap_loss.reshape(2, -1)[0].mean()
            text_cap_loss = cap_loss.reshape(2, -1)[1].mean()

        if not self.opt.enable_text_reconstruction:
    ...
    def caption_prediction_eval(self, cap_head, dt, hs, reference, others, decoder_type, indices=None):
        ...
        if decoder_type in ['none']:
            ...
            event = None
            if self.opt.enable_text_reconstruction:
                clip = self.visual_pre_cap_proj(clip)
            seq, cap_prob_eval = cap_head.sample(event, clip, clip_mask)
            if len(seq):
                seq = seq.reshape(-1, N_q, seq.shape[-1])
                cap_prob_eval = cap_prob_eval.reshape(-1, N_q, cap_prob_eval.shape[-1])
            ...
            if self.opt.enable_text_reconstruction:
                prefix = self.visual_pre_cap_proj(hs_r)
                pass
            else:
                prefix = hs_r
...
class PostProcess(nn.Module):
    ...
    def forward_grounding(self, outputs, target_sizes, targets):
        ...
        #         indices, _ = self.grounding_matcher(aux_outputs, targets)
        #         aux_indices.append(indices)
        #         aux_results.append(get_results(indices, C))
        #     return last_results, aux_results, C
    ...
    def forward(self, outputs, target_sizes, loader, model=None, tokenizer=None, enable_ranking_by_logit=True):
        ...
        if self.opt.enable_contrastive and self.opt.eval_enable_matching_score:
            ...
            # for i in range(len(text_encoder_inputs)):
            #     encoder_input = text_encoder_inputs[i]
            #     i fisinstance(encoder_input, torch.Tensor):
            #         text_encoder_inputs[i] = encoder_input.to(self.opt.device)
            #     else:
            #         for key, val in encoder_input.items():
            #             if isinstance(val, torch.Tensor):
            #                 encoder_input[key] = val.to(self.opt.device)
...
def build(args):
    ...
    weight_dict = {'loss_ce': args.cls_loss_coef,
                   ...
                   'loss_selector': args.selector_loss_coef,
                   }
