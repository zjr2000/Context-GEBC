...
class ShowAttendTellCore(nn.Module):
    ...
    def __init__(self, opt):
        ...
        if vars(self.opt).get('enable_pos_emb_for_captioner', False):
            self.input_dim = opt.hidden_dim * 3
        ...
        self.deformable_att = MSDeformAttnCap(d_model, self.n_levels, self.n_heads, self.n_points, opt)
    ...
    def forward(self, xt, state, query, reference_points, input_flatten, input_spatial_shapes, input_level_start_index, input_padding_mask):
        ...
        memory = state[0][-1].reshape(N_, Lq_, -1)
        joint_query = torch.cat((memory, query), 2)
        xt = xt.reshape(N_, Lq_, xt.shape[-1])
        # (N_ * M_, D_, Lq_, L_* P_)
        ...
        att_res = att_res.reshape(N_, Lq_, self.n_heads, self.att_feat_size).flatten(2)
        input_feats = torch.cat((att_res, query), 2)
        # print(xt.shape, input_feats.shape, query.shape, reference_points.shape)
        output, state = self.rnn(torch.cat([xt, input_feats], 2).reshape(1, N_*Lq_, -1), state)
        output = output[-1] # hidden state of the last layer
        return output, state
